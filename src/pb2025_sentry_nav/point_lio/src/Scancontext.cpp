...
// =================================================================================
// [新增] 保存 ScanContext 的实现代码 (请添加到 Scancontext.cpp 文件末尾)
// =================================================================================

#include <fstream>
#include <iostream>

void SCManager::saveScanContextAndKeys(const std::string &save_dir, const int &index)
{
    // 1. 检查索引是否合法
    if (index < 0 || index >= polarcontexts_.size()) {
        std::cout << "[SCManager] Error: Index " << index << " out of bounds." << std::endl;
        return;
    }

    // 2. 获取对应的描述子矩阵
    Eigen::MatrixXd sc = polarcontexts_[index];

    // 3. 构建文件名 (例如: 1.scd)
    std::string file_path = save_dir + "/" + std::to_string(index) + ".scd";

    // 4. 打开文件并写入
    std::ofstream scd_file(file_path);
    if (scd_file.is_open()) {
        // 逐行逐列写入数据，空格分隔
        ...
        std::cout << "[SCManager] Error: Cannot open file " << file_path << std::endl;
    }
}
