...
std::vector<KeyFrameInfo> sc_keyframes;
Eigen::Vector3d last_sc_kf_pos(0,0,0);
bool is_first_sc_kf = true;
const double SC_KF_DIST_THRESH = 1.0; // 关键帧选取阈值：1米
...
inline void dump_lio_state_to_log(FILE *fp)
{
    ...
    if (use_imu_as_input) {
        ...
        fprintf(fp, "%lf %lf %lf ", kf_input.x_.gravity(0), kf_input.x_.gravity(1), kf_input.x_.gravity(2)); // Gravity
    } else {
        ...
        fprintf(fp, "%lf %lf %lf ", kf_output.x_.gravity(0), kf_output.x_.gravity(1), kf_output.x_.gravity(2)); // Gravity
    }
    fprintf(fp, "\r\n");
    fflush(fp);
}
...
int main(int argc, char **argv)
{
    ...
    heartbeat_ = HeartBeatPublisher::create(nh.get());
                ...
                if (!time_seq.empty()) {
                    ...
                    for (k = 0; k < time_seq.size(); k++) {
                        ...
                        if (publish_odometry_without_downsample) {
                            ...
                            // ================= [新增] ScanContext 关键帧逻辑 (分支1) =================
                            {
                                Eigen::Matrix3d rot_curr = kf_output.x_.rot;
                                Eigen::Vector3d pos_curr = kf_output.x_.pos;
                                double dist = (pos_curr - last_sc_kf_pos).norm();
                                if (is_first_sc_kf || dist > SC_KF_DIST_THRESH) {
                                    Eigen::Matrix4d pose_mat = Eigen::Matrix4d::Identity();
                                    pose_mat.block<3,3>(0,0) = rot_curr;
                                    pose_mat.block<3,1>(0,3) = pos_curr;
                                    
                                    // --- 修正开始 ---
                                    pcl::PointCloud<pcl::PointXYZI>::Ptr sc_cloud(new pcl::PointCloud<pcl::PointXYZI>());
                                    pcl::copyPointCloud(*feats_undistort, *sc_cloud);
                                    scManager.makeAndSaveScancontextAndKeys(*sc_cloud);
                                    // --- 修正结束 ---

                                    KeyFrameInfo kf_info;
                                    kf_info.time = Measures.lidar_beg_time;
                                    kf_info.pose = pose_mat;
                                    sc_keyframes.push_back(kf_info);
                                    last_sc_kf_pos = pos_curr;
                                    is_first_sc_kf = false;
                                }
                            }
                ...
                if (!time_seq.empty()) {
                    ...
                    for (k = 0; k < time_seq.size(); k++) {
                        ...
                        if (publish_odometry_without_downsample) {
                            ...
                            // ================= [新增] ScanContext 关键帧逻辑 (分支2) =================
                            {
                                Eigen::Matrix3d rot_curr = kf_input.x_.rot;
                                Eigen::Vector3d pos_curr = kf_input.x_.pos;
                                double dist = (pos_curr - last_sc_kf_pos).norm();
                                if (is_first_sc_kf || dist > SC_KF_DIST_THRESH) {
                                    Eigen::Matrix4d pose_mat = Eigen::Matrix4d::Identity();
                                    pose_mat.block<3,3>(0,0) = rot_curr;
    ...
    // 只要 flg_exit 为 true (Ctrl+C触发) 或者 pcd_save_en 为 true 且有数据，就尝试保存
    // 这里的逻辑是：如果用户设了 pcd_save_en=True，程序结束时肯定要保存。
    if (!pcl_wait_save->empty() && pcd_save_en) {
        
        // 1. 动态获取保存路径: ~/PointLIO_Output
        const char* home_dir = getenv("HOME");
        std::string base_path;
        
        if (home_dir) {
            base_path = std::string(home_dir) + "/PointLIO_Output";
        } else {
            // 如果获取不到 HOME (极少见), 回退到当前目录
            base_path = "PointLIO_Output"; 
        }

        RCLCPP_INFO(LOGGER, "Saving data to: %s", base_path.c_str());

        // 2. 递归创建目录 (使用系统命令，简单粗暴有效)
        // 创建 .../PCD/ScanContext/SCD
        std::string cmd_mkdir = "mkdir -p " + base_path + "/PCD/ScanContext/SCD";
        int status = system(cmd_mkdir.c_str());
        if (status != 0) {
            RCLCPP_ERROR(LOGGER, "Failed to create directory: %s", base_path.c_str());
        }

        try {
            // --- 保存 PCD ---
            std::string pcd_file = base_path + "/PCD/scans.pcd";
            RCLCPP_INFO(LOGGER, "Writing PCD: %s ...", pcd_file.c_str());
            pcl::PCDWriter pcd_writer;
            pcd_writer.writeBinary(pcd_file, *pcl_wait_save);
            
            // --- 保存 ScanContext ---
            std::string sc_root_dir = base_path + "/PCD/ScanContext";
            std::string scd_dir = sc_root_dir + "/SCD";
            std::string poses_file = sc_root_dir + "/poses.txt";

            RCLCPP_INFO(LOGGER, "Writing ScanContext Data ...");
            
            std::ofstream pose_ofs(poses_file);
            if (pose_ofs.is_open()) {
                pose_ofs << std::fixed << std::setprecision(6);
                
                int total_frames = sc_keyframes.size();
                for (size_t i = 0; i < total_frames; ++i) {
                    // A. 写 Pose
                    Eigen::Matrix4d &p = sc_keyframes[i].pose;
                    pose_ofs << sc_keyframes[i].time << " " 
                             << p(0,0) << " " << p(0,1) << " " << p(0,2) << " " << p(0,3) << " "
                             << p(1,0) << " " << p(1,1) << " " << p(1,2) << " " << p(1,3) << " "
                             << p(2,0) << " " << p(2,1) << " " << p(2,2) << " " << p(2,3) << std::endl;

                    // B. 写 SCD
                    scManager.saveScanContextAndKeys(scd_dir, i);
                    
                    // 简单的进度显示 (每100帧打印一次，防止刷屏)
                    if (i % 100 == 0) {
                         RCLCPP_INFO(LOGGER, "Saved %zu / %d frames", i, total_frames);
                    }
                }
                pose_ofs.close();
                RCLCPP_INFO(LOGGER, "ScanContext saved successfully!");
            } else {
                RCLCPP_ERROR(LOGGER, "Cannot open file: %s", poses_file.c_str());
            }

        } catch (const std::exception& e) {
             RCLCPP_ERROR(LOGGER, "Exception during saving: %s", e.what());
        }
    }
