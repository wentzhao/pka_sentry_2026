...
void SerialDriverNode::init() 
{
  ...
  // rfid_status_pub_ = this->create_publisher<rm_interfaces::msg::RfidStatus>(
  //   "rfid_status", rclcpp::SensorDataQoS());
...
game_status_pub_ = this->create_publisher<rm_interfaces::msg::GameStatus>(
  "game_status", 
  rclcpp::QoS(rclcpp::SensorDataQoS()).reliable()  // 关键修改
);
...
void SerialDriverNode::listenLoop() 
{
  ...
  rm_interfaces::msg::GameStatus game_status_data;
  rm_interfaces::msg::RfidStatus rfid_status_data;
  rm_interfaces::msg::RobotStatus robot_status_data;
  while (rclcpp::ok()) 
  {
    ...
    if (protocol_->receive(receive_data)) 
    {
      ...
      game_status_data.game_progress = receive_data.game_progress;
      game_status_pub_->publish(game_status_data);

      rfid_status_data.friendly_supply_zone_non_exchange = receive_data.friendly_supply_zone_non_exchange;
      rfid_status_pub_->publish(rfid_status_data);

      robot_status_data.current_hp = receive_data.current_hp;
      robot_status_pub_->publish(robot_status_data);
    ...
    //   auto roll = 0.0;
    //   auto pitch = receive_data.pitch; //原本有负号
    //   auto yaw = 0.0;
    //   tf2::Quaternion q;
